...
| EMBED-004 | [x] | Keep managed embed controls and their video mutations unavailable until the authenticated bridge and the active concrete video report the exact same media identity. Resume now latches only after an identity-matched seek, playing, and final-position acknowledgement; makes at most three attempts across the entire navigation; never rewinds a paused video already past the target; and carries the pending target through an explicit quality navigation without trusting a transient zero tick. Any same-document replacement B is fail-closed and cannot inherit A's position or be auto-played, even while A's restore is unresolved. Every later exact-video manual intent invalidates pending native and in-page resume work; automatic AniSkip seeks defer to the farther pending restore target instead of erasing saved progress. Lifecycle pause/stop gates resume and skip dispatch, cancels native retries, and reconciles stale page play promises so ON_RESUME cannot resurrect playback. The pre-tick window retains the honest provider handoff; seek, play/pause, speed, caption, and web-volume paths cannot act on an unidentified video. | `fix/embed-concrete-control-readiness` |
