...
| HISTORY-009 | [x] | Keep the selected embed document URL as the in-memory persistence identity while using a separate opaque AniSkip cache identity. Manual Next, source changes, fullscreen/TV exit, and player close now flush the latest concrete embed position, while an outgoing in-document video instance cannot be saved after its replacement becomes active. Signed document and video URLs remain redacted from diagnostics. | `fix/embed-progress-save-identity` |
