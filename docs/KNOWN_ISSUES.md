...
| ID | Status | Fix | Topic branch |
| --- | --- | --- | --- |
| DIAG-001 | [x] | Redact signed playback and WebView URLs from user-shareable diagnostics and debug logs. URL-bearing lifecycle, resolver, navigation, and native-player entries retain only a bounded normalized host and stable SHA-256 fingerprint. Remote page titles, WebView error text, HTTP reason phrases, pipe response bodies, and player exception messages are excluded from logs while user-facing errors remain available in the UI. | `fix/embed-diagnostic-url-redaction` |
