...
| EMBED-004 | [x] | Keep managed embed controls and their runtime mutations unavailable until the authenticated bridge and the active concrete video report the exact same media identity. The pre-tick readiness window now retains the honest provider handoff; seek, play/pause, speed, caption, and web-volume paths cannot act on an unidentified video. | `fix/embed-concrete-control-readiness` |
