...
| EMBED-004 | [x] | Keep managed embed controls and their video mutations unavailable until the authenticated bridge and the active concrete video report the exact same media identity. Resume now latches only after an identity-matched seek-and-playing acknowledgement, makes at most three attempts with delayed retries after failures/timeouts, and gives a replacement video its own current-position opportunity without accepting a stale command. The pre-tick window retains the honest provider handoff; seek, play/pause, speed, caption, and web-volume paths cannot act on an unidentified video. | `fix/embed-concrete-control-readiness` |
