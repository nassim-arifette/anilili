...
class WatchViewModel : ViewModel() {
    ...
    /** Progress from WebView players, which do not expose a Media3 item identity. */
    fun onProgress(positionMs: Long, durationMs: Long) {
        ...
        acceptProgress(
            ...
            totalEpisodesSnapshot = totalEpisodes,
        )
    }

    /** Native progress must identify the MediaItem that actually produced the callback. */
    fun onNativeProgress(identity: PlaybackIdentity, positionMs: Long, durationMs: Long) {
        val data = (_state.value as? UiState.Success)?.data ?: return
        ...
        acceptProgress(identity, positionMs, durationMs, totalEpisodes)
    }
    ...
    private fun acceptProgress(
        ...
        maybeSyncAniListProgress(identity, positionMs, durationMs, totalEpisodesSnapshot)
